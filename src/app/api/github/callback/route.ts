...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!params.success) {
      console.error('Validation error for GitHub callback parameters:', params.error);
    }
    ...
    try {
      ...
      try {
        if (session?.user?.id) {
          ...
          console.log(`Successfully saved GitHub tokens for user ${session.user.id}`);
        }
      } catch (dbError) {
        console.error('Error saving GitHub tokens to database:', dbError);
      }
      
      return NextResponse.redirect(new URL('/setup?github_connected=true', req.url));
    } catch (error: unknown) {
